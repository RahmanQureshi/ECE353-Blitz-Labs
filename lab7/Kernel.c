  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
		...
		while true
			...
			if currentChunkSize <= 0
				break
			endIf
			if virtPage < 0 || virtPage > currentThread.myProcess.addrSpace.numberOfPages || currentThread.myProcess.addrSpace.IsValid(virtPage)==false || currentThread.myProcess.addrSpace.IsWritable(virtPage)==false
				return -1 -- error			
			endIf
